...
def check_intersections(maze, path, end_tile):
    ...
    for node in path:
        ...
        for successor in successors:
            ...
            alt_path = a_star_search(maze, successor, end_tile, True, node[0])

            # if alt_path[0][0] == path[0][0]:
...
def a_star_search(maze, start_tile, end_tile, alt=False, branch_parent=None):
    """
    ...
    """
    if branch_parent:
        t_node = {'pos': branch_parent}
        open_list = [{'pos': start_tile, 'char': maze[start_tile], 'f': 0, 'g': 0, 'parent': t_node}]
    else:
    ...
    while open_list:
        ...
        for successor in successors:
            ...
            if maze[successor['pos']] == 'E':
                ...
                successor['g'] = q['g'] + (
                    calculate_distance(successor, q) if not alt else calculate_distance_alt(successor, q, branch_parent))
...
def calculate_distance_alt(node, parent, branch_parent):

    orig_parent = parent['parent']

    grand_parent = {'pos': branch_parent}
    parent['parent'] = grand_parent

    distance = calculate_distance(node, parent)
    parent['parent'] = orig_parent
...
def calculate_distance(node, parent):
    ...
    if abs(current_direction - original_direction) == 2:
        return 2001
    elif current_direction != original_direction:
        return 1001
    else:
        return 1
